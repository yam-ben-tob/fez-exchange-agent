...
and two tools: Pinecone factsheet search + DuckDuckGo web search.
"""
import json
import re
...
def _search_factsheets(university: str, query: str) -> str:
    """Search Pinecone factsheets for course/department info at a university."""
    results = query_embedding(query, top_k=3, filter={"university": {"$eq": university}})
    texts = [
...
def _search_web(query: str) -> str:
    """Search the web for course catalog or exchange program info."""
    try:
...
TOOLS = {
    "search_factsheets": _search_factsheets,
...
def _react_loop(system_prompt: str, user_prompt: str, max_iter: int = 8) -> str:
    """
